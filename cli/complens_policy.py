...
def cmd_list(a):
    ...
    for p in eng.list_policies():
        ...
        extra = []
        if p["rules"] > 1: extra.append(f"{p['rules']} rules")
        if p["has_escalation"]: extra.append("escalation")
        if p["requires"]: extra.append("requires " + ",".join(p["requires"]))
...
def main():
    ...
    pt = sub.add_parser("test"); pt.add_argument("-v", "--verbose", action="store_true")
    pe = sub.add_parser("eval"); pe.add_argument("--control", required=True); pe.add_argument("--evidence", default="{}")
    sub.add_parser("list")
    a = ap.parse_args()
