...
from app.connectors.http_client import (
    ResilientClient, _is_blocked_host, _redact, _breaker_for, _breakers)
...
def test_retries_then_succeeds():
    ...
    def fake(method, url, **kw):
        calls["n"] += 1
        m = MagicMock(); m.headers = {}
        if calls["n"] < 3:
            m.status_code = 503; m.text = "busy"
        else:
            m.status_code = 200; m.json = lambda: {"ok": True}
        return m
...
def test_429_honored():
    calls = {"n": 0}
    def fake(method, url, **kw):
        calls["n"] += 1; m = MagicMock()
        if calls["n"] == 1:
            m.status_code = 429; m.headers = {"Retry-After": "0"}; m.text = "rl"
        else:
            m.status_code = 200; m.headers = {}; m.json = lambda: {"ok": 1}
        return m
...
def test_error_redacts_leaked_creds():
    def leak(method, url, **kw):
        m = MagicMock(); m.status_code = 401
        m.text = '{"error":"bad: SSWS leaked123"}'; m.headers = {}; return m
